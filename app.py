...
async def show_hand(ctx):
    ...
    if uid in game_state["players"]:
        ...
        await ctx.send(f"{ctx.author.name}'s hand: {hand_str}\nBuildings: {build_str}")
    else:
...
@bot.command()
async def build(ctx, structure: str, number: int, resource: str):
    uid = str(ctx.author.id)
    if uid not in game_state["players"]:
    ...
    cost_table = {
        ...
        "city":       {"wheat":2,"ore":3},
        "fortress":   {r:2 for r in ["wood","brick","sheep","wheat","ore"]},
        "road":       {"wood":1,"brick":1}
    }

    struct = structure.lower()
    res = resource.lower()
    if struct not in cost_table or res not in resources:
        return await ctx.send("Invalid structure or resource.")

    cost = cost_table[struct]
    if not has_resources(p["hand"], cost):
        return await ctx.send(f"Not enough resources to build a {struct}.")

    # Deduct cost
    deduct_resources(p["hand"], cost)

    # Register building under specific roll and resource
    dice_key = str(number)
    if dice_key not in p["buildings"]:
        p["buildings"][dice_key] = {}
    if res not in p["buildings"][dice_key]:
        p["buildings"][dice_key][res] = {"settlement": 0, "city": 0, "fortress": 0}
    p["buildings"][dice_key][res][struct] = p["buildings"][dice_key][res].get(struct, 0) + 1

    save_state(game_state)
    await ctx.send(f"{ctx.author.name} built a {struct} on {number} for {res}.")
    ...
    for uid, player in game_state["players"].items():
        ...
        if total in buildings:
            for resource, btypes in buildings[total].items():
                amount = (
                    btypes.get("settlement", 0) * 1 +
                    btypes.get("city", 0) * 2 +
                    btypes.get("fortress", 0) * 3
                )
                hand[resource] += amount
                if amount > 0:
                    output.append(f"{player['name']} gains {amount} {resource}")
                    resource_yielded = True

        if not resource_yielded:
            hand["gold"] += 1
            output.append(f"{player['name']} has no buildings on {total} → +1 gold")
...
async def fight(ctx):
    ...
    if user_id not in [ab["attacker"]]:  # If you want defender too, add their ID here when known
        return await ctx.send("Only the attacker can resolve the battle for now.")
    ...
    for i in range(comparisons):
        ...
        if d >= a:
            a_losses += 1
            report.append(f"Defender {d} beats Attacker {a} → Attacker loses 1 troop")
        else:
            d_losses += 1
            report.append(f"Attacker {a} beats Defender {d} → Defender loses 1 troop")
    ...
    if ab["attacker_count"] == 0:
        ...
        save_state(game_state)
        report.append(f"Remaining troops — Attacker: {ab['attacker_count']} | Defender: {ab['defender_count']}")
